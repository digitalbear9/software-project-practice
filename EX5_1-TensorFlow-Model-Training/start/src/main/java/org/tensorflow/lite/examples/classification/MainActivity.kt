...
//        }
        ...
        private val flowerModel: FlowerModel by lazy {
            ...
            val options = if (compatList.isDelegateSupportedOnThisDevice) {
                Log.d(TAG, "当前模型运行设备：GPU")

                Model.Options.Builder()
                    .setDevice(Model.Device.GPU)
                    .build()            } else {
                Log.d(TAG, "当前模型运行设备：CPU，因为该设备不支持 GPU Delegate")
        ...
        // 花卉类别标签，顺序必须和训练时的 class_names / labels.txt 一致
        ...
        // 图片预处理器：把摄像头图片统一缩放到模型需要的 224 × 224
...
//            val outputs = flowerModel.process(tfImage.tensorBuffer)
//                .outputs.outputFeature0AsTensorBuffer.floatArray.apply {
...
//        }
        ...
        override fun analyze(imageProxy: ImageProxy) {
            try {
                ...
                // 1. 将 CameraX 图像转成 Bitmap
                val bitmap = toBitmap(imageProxy) ?: return

                // 2. 将 Bitmap 转成 TensorImage
                ...
                // 4. 调用模型
                // 注意：这里传入的是 tfImage.tensorBuffer，而不是 tfImage
                val outputs = flowerModel.process(tfImage.tensorBuffer)

                // 5. 取出模型输出的概率数组
                val probabilities = outputs.outputFeature0AsTensorBuffer.floatArray

                // 6. 将概率和标签对应起来，并按置信度从高到低排序
                val results = probabilities
                    .mapIndexed { index, score ->
                        labels.getOrElse(index) { "class_$index" } to score
                    }
                    .sortedByDescending { it.second }
                    .take(MAX_RESULT_DISPLAY)

                // 7. 转成界面需要的 Recognition 类型
